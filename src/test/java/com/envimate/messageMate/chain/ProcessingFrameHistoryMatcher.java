...
final class ProcessingFrameHistoryMatcher {
    ...
    ProcessingFrameHistoryMatcher withAFrameFor(final Chain<TestMessage> chain, final Class<? extends Action> actionClass) {
        final ExpectedProcessingFrame expectedProcessingFrame = new ExpectedProcessingFrameForActionClass(chain, actionClass);
        expectedProcessingFrames.add(expectedProcessingFrame);
        return this;
    }

    @SuppressWarnings("rawtypes")
    ProcessingFrameHistoryMatcher withAFrameFor(final Chain<TestMessage> chain, final Action<TestMessage> expectedAction) {
        final ExpectedProcessingFrame expectedProcessingFrame = new ExpectedProcessingFrameForAction(chain, expectedAction);
        expectedProcessingFrames.add(expectedProcessingFrame);
        return this;
    }

    void assertCorrect(final ProcessingContext<TestMessage> processingContext) {
        ChainProcessingFrame<TestMessage> currentProcessingFrame = processingContext.getInitialProcessingFrame();
    ...
    private void assertFirstProcessingFrameCorrect(final ChainProcessingFrame<TestMessage> firstProcessingFrame) {
        final ChainProcessingFrame<TestMessage> previousFrame = firstProcessingFrame.getPreviousFrame();
        assertThat(previousFrame, equalTo(null));
    }

    private void assertDoubleLinkedListCorrectOf(final ChainProcessingFrame<TestMessage> currentProcessingFrame) {
        final ChainProcessingFrame<TestMessage> previousFrame = currentProcessingFrame.getPreviousFrame();
        if (previousFrame != null) {
            assertThat(previousFrame.getNextFrame(), equalTo(currentProcessingFrame));
        }
        final ChainProcessingFrame<TestMessage> nextFrame = currentProcessingFrame.getNextFrame();
        if (nextFrame != null) {
            assertThat(nextFrame.getPreviousFrame(), equalTo(currentProcessingFrame));
        }
    }

    private void assertCorrectnessOfCallOrReturnIfPresent(final ChainProcessingFrame<TestMessage> currentProcessingFrame) {
        final Action<TestMessage> action = currentProcessingFrame.getAction();
        final Call<TestMessage> callAction;
        final ChainProcessingFrame<TestMessage> callProcessingFrame;
        final Return<TestMessage> returnAction;
        final ChainProcessingFrame<TestMessage> returnProcessingFrame;
    ...
    private void assertLastProcessingFrameCorrect(final ChainProcessingFrame<TestMessage> lastProcessingFrame) {
        final ChainProcessingFrame<TestMessage> nextFrame = lastProcessingFrame.getNextFrame();
        assertThat(nextFrame, equalTo(null));
    }


    private interface ExpectedProcessingFrame {
        void assertEquals(ChainProcessingFrame<TestMessage> chainProcessingFrame);
    }

    @RequiredArgsConstructor(access = PRIVATE)
    @SuppressWarnings("rawtypes")
    private final class ExpectedProcessingFrameForActionClass implements ExpectedProcessingFrame {
        private final Chain<TestMessage> expectedChain;
        private final Class<? extends Action> expectedActionClass;


        public void assertEquals(final ChainProcessingFrame<TestMessage> chainProcessingFrame) {
            final Chain<TestMessage> actualChain = chainProcessingFrame.getChain();
            assertThat(actualChain, equalTo(expectedChain));

            final Action<TestMessage> actualAction = chainProcessingFrame.getAction();
